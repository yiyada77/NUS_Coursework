...
% 3. Train the network in sequential mode
    for i = 1 : epochs
        ...
        pred_val = round(net(images(:,train_num+1:train_num+val_num))); % predictions on validation set
        accu_val(i) = 1 - mean(abs(pred_val-labels(train_num+1:train_num+val_num)));
    end
end